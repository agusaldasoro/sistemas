...
\subsubsection*{Función Initialize}		

\textcolor{red}{Para que esta?? si nunca se le otorga ningun comportamineto}
...
Si la tarea que estaba corriendo en ese core se terminó, se bloqueó o era la Idle, se devuelve \texttt{next(cpu)}. \textcolor{red}{La misma devuelve el \emph{pid} de la próxima tarea a ejecutar en \emph{cpu}.}

Sino voy a actualizar el \texttt{quantums\_timer} (restarle 1) en la posición correspondiente al cpu. Si el timer 
...
Si la tarea que se estaba ejecutando no termin\'o su ejecuci\'on o se bloqueó, se deber\'a encolar en la cola global de pendientes.
...
\textcolor{red}{En principio esto va a depender del lote de tareas, la idea es usar mejor el CPU, puede que no se use al 100\% siempre. Nunca va a haber idle time antes de un overflow.}\\
...
priority_queue<datos> pq;
	...
	\begin{itemize}
	\item[•]\textbf{struct datos} Diseñado para representar a una tarea, en él se podra encontrar el PID y el período de la misma.\textcolor{red}{¿Por qué no se llama ``tarea'' entonces?}
...
\textcolor{red}{La manera en la que resuelven la elección de los lugares donde hacer los bloqueos hace que tarde mucho en ejecutarse el simulador. Mientras más total_cpu le asignan a una tarea peor es. Deberían pensar otra manera de distribuir esos bloqueos y no con ``prueba y error'' (línea 65 de task.cpp)}
...
\textcolor{red}{Pero con quantums muy chicos perdes muchos clocks en context switch, ojo con esto}
...
\textcolor{red}{ Tampoco implementan la función unblock()} 
